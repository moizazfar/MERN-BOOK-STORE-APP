...
import { loginUser } from "../redux/slices/auth/controller";

const Signin = () => {
  const dispatch = useDispatch();

  const { isError, errorMessage, user, isSuccess, isLoading } = useSelector(
    (state) => state.auth
  );
  ...
  useEffect(() => {
    if (user != null && isSuccess && !isLoading) {
      navigate("/admin/dashboard");
    } else if (!isLoading && !isSuccess && isError) {
      alert(errorMessage);
    }
  }, [user, isSuccess, isLoading, isError]);
